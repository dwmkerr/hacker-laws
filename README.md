...
As can be seen, even a program which has 50% of the program which can run in parallel will benefit very little beyond 10 processing units, where as a program which is 95% parallelisable can still achieve significant speed improvements with over a thousand processing units.

As [Moore's Law](#TODO) slows, and the acceleration of individual processor speed slows, parallelization is key to improving performance. Many modern computing solutions, such as [Ray Tracing](https://en.wikipedia.org/wiki/Ray_tracing_(graphics)) are extremely parallelizable. In the ray-tracing example, _every pixel_ on a monitor can be processed in parallel, which is one of the reasons that modern graphics cards have extremely large numbers of processors (GPUs/Shader Units).
